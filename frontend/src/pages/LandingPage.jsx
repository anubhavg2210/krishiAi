...
export default function LandingPage() {
  ...
  const features = featureConfig.map((feature) => {
    const cards = t(`${feature.textKey}.cards`, []);
    return {
      ...feature,
      pill: t(`${feature.textKey}.pill`),
      titleTop: t(`${feature.textKey}.titleTop`),
      titleBottom: t(`${feature.textKey}.titleBottom`),
      description: t(`${feature.textKey}.description`),
      primaryLinkText: t(`${feature.textKey}.cta`),
      miniCards: cards.map(([title, desc], index) => ({
        icon: feature.icons[index],
        title,
        desc,
      })),
    };
    ...
    <div className="flex flex-col items-center w-full pb-12">
      ...
      <div className="w-full mt-24 pb-12 flex flex-col items-center justify-start gap-8 relative text-center z-10">
        <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight leading-[1.2] max-w-4xl mx-auto">
          <span className="text-[#4CAF50]">{t("landing.headlinePrefix")} </span>{t("landing.headline")}
        </h2>
